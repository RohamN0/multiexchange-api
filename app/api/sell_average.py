...
@router.get('/sell_average')
async def sell_avg(credentials: Annotated[HTTPBasicCredentials, Depends(security)]):
    usdt_to_irt_response = aban_response_taker('/api/v1/otc/coin-price?coin=USDT')
    aban_sell_response = aban_response_taker('/api/v1/otc/orders/sell/?page=1&count=10')
    okex_sell_response = okex_response_taker('/oapi/v1/trade/otc/history?side=sell', 'GET')

    currency = { data['asset'] : { 'amount': [], 'coinPrice_irt': [], 'irtPrice': [], 
            'avrg_amount': 0, 'avrg_coinPrice_irt': 0, 
            'avrg_irtPrice': 0 } for data in okex_sell_response['data'] }
    currency.update({ data['coinSymbol'] : 
        { 'amount': [], 'coinPrice_usdt': [], 'coinPrice_irt': [], 'usdtPrice': [], 
         ...
         'avrg_tetherPrice': 0 } for data in aban_sell_response['orders'] })

    for data in aban_sell_response['orders'] :
    ...
    for data in okex_sell_response['data'] :
